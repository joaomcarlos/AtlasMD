---
title: Mermaid Diagrams
description: AtlasMD renders mermaid code blocks as interactive SVG diagrams with click-to-zoom.
---

# Mermaid Diagrams

AtlasMD renders mermaid code blocks as interactive SVG diagrams with click-to-zoom. The renderer initializes mermaid with theme-aware colors (light and dark) and re-renders on theme switch.

## 1. Examples

::side-note
Examples
::

### Flowchart

```mermaid
flowchart LR
    A[Consumer Project] -->|volume mount| B[AtlasMD Image]
    B -->|serves| C[Browser]
```

### Sequence Diagram

```mermaid
sequenceDiagram
    participant C as Consumer
    participant A as AtlasMD
    participant N as Nuxt
    C->>A: docker compose up
    A->>N: start dev server
    N->>N: read mounted content/
    N-->>C: serve documentation site
```

### State Diagram

```mermaid
stateDiagram-v2
    [*] --> Building: docker compose build
    Building --> Running: docker compose up
    Running --> Stopped: docker compose down
    Stopped --> [*]
```

## 2. Usage

Use a fenced code block with the `mermaid` language tag:

````markdown
```mermaid
flowchart LR
    A --> B --> C
```
````

## 3. Features

- **Theme-aware**: Mermaid re-initializes with light or dark colors when the user toggles theme
- **Click-to-zoom**: Click the diagram to open a full-screen modal; press `Escape` or click outside to close
- **Supported languages**: `python`, `mermaid`, `jsonc` are registered in `nuxt.config.ts` under `mdc.highlight.langs`

## 4. Source

`atlasmd-renderer/components/content/ProseCode.vue` — detects `language === 'mermaid'`, dynamically imports the mermaid library, renders the SVG, and handles the zoom modal. Theme variables are defined inline for both light and dark modes.

## 5. Next Steps

- [Fig](/building-blocks/fig) — Image component with dark mode variants and click-to-zoom
- [Docus Built-in Components](/building-blocks/docus-builtins) — Landing page and card grid components from Docus
- [Footnotes](/building-blocks/footnotes) — Markdown footnotes with superscript numbers and references list
